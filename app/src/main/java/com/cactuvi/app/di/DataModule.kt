 ...
 * All provided dependencies are @Singleton (single instance per app)
 */
@Module
...
object DataModule {
     ...
     * Uses existing AppDatabase.getInstance() pattern.
     */
    @Provides
    @Singleton
    ...
    /**
     * Provide base URL from active source.
     * Returns active source's server URL or default placeholder.
     * 
     * Note: This is re-evaluated per-injection. In Phase 2, we'll refactor
     * to handle dynamic source switching properly.
     */
    @Provides
    @ActiveSourceUrl
    fun provideActiveSourceUrl(
        ...
        return runBlocking {
            sourceManager.getActiveSource()?.server ?: "http://placeholder.local"
        }
    }
     ...
     * 
     * Singleton scope means this uses the URL from app startup.
     * TODO Phase 2: Refactor to support dynamic source switching without restarting app.
     */
    @Provides
    @Singleton
    ...
    @Provides
    @Singleton
    fun provideLiveLocalDataSource(
        database: AppDatabase
    ): LiveLocalDataSource {
    ...
    @Provides
    @Singleton
    fun provideLiveRemoteDataSource(
        apiService: XtreamApiService
