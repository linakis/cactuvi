...
object OptimizedBulkInsert {
    ...
    /**
     * Insert a batch of movies using multi-value INSERT. Call beginMoviesInsert() once before, and
     * endMoviesInsert() once after all batches.
     */
    ...
    fun endMoviesInsert(database: SupportSQLiteDatabase) {
        ...
        database.execSQL(
            "CREATE UNIQUE INDEX IF NOT EXISTS index_movies_sourceId_streamId ON movies(sourceId, streamId)",
        )
    ...
    fun endSeriesInsert(database: SupportSQLiteDatabase) {
        ...
        database.execSQL(
            "CREATE UNIQUE INDEX IF NOT EXISTS index_series_sourceId_seriesId ON series(sourceId, seriesId)",
        )
    }
    ...
    private fun insertSeriesBatch(database: SupportSQLiteDatabase, series: List<SeriesEntity>) {
        ...
        try {
            ...
            series.forEach { s ->
                ...
                if (s.cover != null) statement.bindString(bindIndex++, s.cover)
                else statement.bindNull(bindIndex++)
                if (s.plot != null) statement.bindString(bindIndex++, s.plot)
                else statement.bindNull(bindIndex++)
                if (s.cast != null) statement.bindString(bindIndex++, s.cast)
                else statement.bindNull(bindIndex++)
                if (s.director != null) {
                    statement.bindString(
                        bindIndex++,
                ...
                if (s.genre != null) statement.bindString(bindIndex++, s.genre)
                else statement.bindNull(bindIndex++)
                ...
                if (s.rating != null) statement.bindString(bindIndex++, s.rating)
                else statement.bindNull(bindIndex++)
                if (s.rating5Based != null) {
                    statement.bindDouble(
                        bindIndex++,
                        s.rating5Based,
                    )
                ...
                if (s.backdropPath != null) {
                    statement.bindString(
                        bindIndex++,
                        gson.toJson(s.backdropPath),
                    )
                } else {
                    statement.bindNull(bindIndex++)
                }
                if (s.youtubeTrailer != null) {
                    statement.bindString(
                        bindIndex++,
                        s.youtubeTrailer,
                    )
    ...
    fun insertLiveChannelBatchOptimized(
    ...
    fun endLiveChannelsInsert(database: SupportSQLiteDatabase) {
        database.execSQL(
            "CREATE INDEX IF NOT EXISTS index_live_channels_categoryId ON live_channels(categoryId)"
        )
        database.execSQL(
            "CREATE UNIQUE INDEX IF NOT EXISTS index_live_channels_sourceId_streamId ON live_channels(sourceId, streamId)",
        )
    }

    private fun insertLiveChannelBatch(
        database: SupportSQLiteDatabase,
        channels: List<LiveChannelEntity>
    ) {
        val sql = buildString {
        ...
        try {
            ...
            channels.forEach { channel ->
                ...
                if (channel.tvArchive != null) {
                    statement.bindLong(
                        bindIndex++,
                        channel.tvArchive.toLong(),
                    )
                } else {
                    statement.bindNull(bindIndex++)
                }
                if (channel.directSource != null) {
                    statement.bindString(
                        bindIndex++,
                ...
                if (channel.tvArchiveDuration != null) {
                    statement.bindString(
                        bindIndex++,
                        channel.tvArchiveDuration,
                    )
                } else {
                    statement.bindNull(bindIndex++)
                }
