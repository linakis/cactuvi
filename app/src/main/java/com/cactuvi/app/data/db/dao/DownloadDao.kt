...
interface DownloadDao {
    ...
    @Insert(onConflict = OnConflictStrategy.REPLACE) suspend fun insert(download: DownloadEntity)

    @Update suspend fun update(download: DownloadEntity)

    @Query(
        "UPDATE downloads SET status = :status, progress = :progress, bytesDownloaded = :bytesDownloaded WHERE contentId = :contentId",
    )
    ...
    @Query(
        "UPDATE downloads SET status = :status, downloadUri = :downloadUri, progress = 1.0, completedAt = :completedAt WHERE contentId = :contentId",
    )
    ...
    @Query(
        "UPDATE downloads SET status = :status, failureReason = :failureReason WHERE contentId = :contentId"
    )
    ...
    @Query("DELETE FROM downloads WHERE status = 'completed'") suspend fun deleteCompleted()
}
