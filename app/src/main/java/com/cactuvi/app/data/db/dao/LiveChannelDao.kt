...
interface LiveChannelDao {
    ...
    @Query("DELETE FROM live_channels WHERE 1") suspend fun clearAll()
    ...
    /**
     * Get count of live channels in cache for fast validation. Used as fallback when metadata table
     * not available.
     */
    @Query("SELECT COUNT(*) FROM live_channels") suspend fun getCount(): Int
     ...
     */
    @Query("SELECT lastUpdated FROM live_channels LIMIT 1") suspend fun getFirstUpdatedTime(): Long?

    /**
     * Get count of live channels in a specific category. Used for displaying category counts
     * without loading all channels.
     */
