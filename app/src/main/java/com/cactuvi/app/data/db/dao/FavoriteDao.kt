...
interface FavoriteDao {
    ...
    @Insert(onConflict = OnConflictStrategy.REPLACE) suspend fun insert(favorite: FavoriteEntity)
    ...
    @Query(
        "SELECT * FROM favorites WHERE sourceId = :sourceId AND contentType = :type ORDER BY addedAt DESC"
    )
