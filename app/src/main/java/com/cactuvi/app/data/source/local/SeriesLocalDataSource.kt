...
/**
 * Local data source for series. Handles all database operations for series.
 *
...
class SeriesLocalDataSource(
    ...
    fun getAllPaged(): PagingSource<Int, SeriesEntity> = database.seriesDao().getAllPaged()

    /** Delete series by source ID */
    suspend fun deleteBySourceId(sourceId: String) = database.seriesDao().deleteBySourceId(sourceId)

    /** Insert series in batch via DbWriter */
    suspend fun insertAll(series: List<SeriesEntity>) = database.getDbWriter().writeSeries(series)

    /** Get cache metadata for series */
    suspend fun getCacheMetadata(): CacheMetadataEntity? = database.cacheMetadataDao().get("series")
