...
object CategoryGrouper {
    ...
    data class NavigationTree(
        ...
        /**
         * Check if navigation should skip categories level within a group (only 1 category in the
         * group)
         */
        ...
        /**
         * Check if navigation should skip both groups and categories levels (only 1 group with only
         * 1 category)
         */
    ...
    fun buildLiveNavigationTree(
        ...
        if (!groupingEnabled) {
            ...
            val filteredCategories =
                filterCategoriesByName(categories, hiddenCategories, filterMode)
        ...
        return filterNavigationTreeHierarchical(
            NavigationTree(grouped),
            hiddenGroups,
            hiddenCategories,
            filterMode
        )
    ...
    /**
     * Build navigation tree for live TV categories with custom separator and filtering (backward
     * compatibility)
     */
    fun buildLiveNavigationTree(
        categories: List<Category>,
    ...
    fun buildVodNavigationTree(
        ...
        PerformanceLogger.log(
            "Input: ${categories.size} categories, grouping=$groupingEnabled, hiddenGroups=${hiddenGroups.size}, hiddenCategories=${hiddenCategories.size}",
        )
        ...
        val tree =
            filterNavigationTreeHierarchical(
                NavigationTree(grouped),
                hiddenGroups,
                hiddenCategories,
                filterMode
    ...
    /**
     * Build navigation tree for VOD categories with custom separator and filtering (backward
     * compatibility)
     */
    fun buildVodNavigationTree(
        categories: List<Category>,
    ...
    /**
     * Build navigation tree for series categories with custom separator and filtering (backward
     * compatibility)
     */
    fun buildSeriesNavigationTree(
        categories: List<Category>,
    ...
    /**
     * Filter navigation tree hierarchically (both groups and categories) First filters entire
     * groups, then filters individual categories within remaining groups
     */
    private fun filterNavigationTreeHierarchical(
        ...
        PerformanceLogger.log(
            "[filterNavigationTreeHierarchical] Input: ${tree.groups.size} groups, mode=$filterMode"
        )

        // If both are empty, no filtering needed
        if (hiddenGroups.isEmpty() && hiddenCategories.isEmpty()) {
            PerformanceLogger.log(
                "[filterNavigationTreeHierarchical] No filtering - both sets empty"
            )
            return tree
        }
        ...
        val fullyFilteredGroups =
            if (hiddenCategories.isEmpty()) {
                ...
                groupFilteredGroups
                    ...
                    } // Remove empty groups after category filtering
            }
        ...
        PerformanceLogger.log(
            "[filterNavigationTreeHierarchical] Output: ${fullyFilteredGroups.size} groups, $totalCategoriesFiltered categories",
        )
