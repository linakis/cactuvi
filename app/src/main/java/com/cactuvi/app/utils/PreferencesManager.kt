...
class PreferencesManager private constructor(context: Context) {
    ...
    fun getMoviesFilterMode(): ContentFilterSettings.FilterMode {
        val mode =
            prefs.getString(KEY_MOVIES_FILTER_MODE, ContentFilterSettings.FilterMode.BLACKLIST.name)
    ...
    fun getSeriesFilterMode(): ContentFilterSettings.FilterMode {
        val mode =
            prefs.getString(KEY_SERIES_FILTER_MODE, ContentFilterSettings.FilterMode.BLACKLIST.name)
                ?: ContentFilterSettings.FilterMode.BLACKLIST.name
        return try {
            ContentFilterSettings.FilterMode.valueOf(mode)
        } catch (e: IllegalArgumentException) {
    ...
    fun getLiveFilterMode(): ContentFilterSettings.FilterMode {
        val mode =
            prefs.getString(KEY_LIVE_FILTER_MODE, ContentFilterSettings.FilterMode.BLACKLIST.name)
                ?: ContentFilterSettings.FilterMode.BLACKLIST.name
        return try {
            ContentFilterSettings.FilterMode.valueOf(mode)
        } catch (e: IllegalArgumentException) {
    ...
    fun getFilterMode(
        contentType: ContentFilterSettings.ContentType
    ): ContentFilterSettings.FilterMode {
        return when (contentType) {
    ...
    fun setFilterMode(
        contentType: ContentFilterSettings.ContentType,
        mode: ContentFilterSettings.FilterMode
    ) {
        when (contentType) {
    ...
    /**
     * Check if VPN warning is enabled (default: false). When enabled, app will warn user if VPN is
     * not connected.
     */
