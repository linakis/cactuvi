 ...
 * 
 * Many IPTV providers bind credentials to specific MAC addresses.
 * This class allows you to configure and use a fixed MAC address
 * instead of the device's actual MAC address.
 */
class MacAddressManager(context: Context) {
    ...
    companion object {
        ...
        @Volatile
        private var instance: MacAddressManager? = null
        
        fun getInstance(context: Context): MacAddressManager {
            return instance ?: synchronized(this) {
                instance ?: MacAddressManager(context.applicationContext).also {
                    instance = it
                }
            }
        }
    ...
    /**
     * Get the configured MAC address to use for API calls.
     * Returns the user-configured MAC or default if not set.
     */
    ...
    /**
     * Set the MAC address to use for API authentication.
     * This should be the MAC address registered with your IPTV provider.
     * 
    ...
    /**
     * Get the configured device key (if any).
     * Some providers use additional device identifiers.
     */
    ...
    /**
     * Get MAC address encoded as Base64 (for certain API calls).
     * IP-Pro encodes MAC in Base64 for some requests.
     */
    ...
    /**
     * Generate a fake Android ID based on MAC address.
     * Some providers check Android ID consistency with MAC.
     */
