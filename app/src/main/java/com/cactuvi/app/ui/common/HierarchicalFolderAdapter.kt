...
class HierarchicalFolderAdapter(
    ...
    private var displayList: List<HierarchicalItem> =
        HierarchicalItemHelper.buildDisplayList(groups)
    ...
    /** Filter displayed items based on search query Auto-expands groups with matching children */
    fun filter(query: String) {
        ...
        for (group in groups) {
            ...
            val matchingCategories =
                group.categories.filter { it.categoryName.lowercase().contains(lowerQuery) }
                            ...
                            HierarchicalItem.CategoryItem(
                                ...
                                isChecked =
                                    displayList
                                        ...
                                        .find { it.name == category.categoryName }
                                        ?.isChecked ?: false,
                            ),
                        )
                    }
