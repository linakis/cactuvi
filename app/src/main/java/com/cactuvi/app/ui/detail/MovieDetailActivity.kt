...
class MovieDetailActivity : AppCompatActivity() {
    ...
    private fun setupClickListeners() {
        ...
        playButton.setOnClickListener { playMovie(startPosition = 0) }

        favoriteButton.setOnClickListener { viewModel.toggleFavorite() }

        downloadButton.setOnClickListener { handleDownloadClick() }

        resumeButton.setOnClickListener { playMovie(startPosition = resumePosition) }
    }
    ...
    private fun renderUiState(state: MovieDetailUiState) {
        ...
        // Movie info
        state.movieInfo?.let { movieInfo -> displayMovieInfo(movieInfo) }
        ...
        state.movieInfo?.movieData?.containerExtension?.let { containerExtension = it }
        state.movieInfo?.movieData?.streamId?.let { streamId = it }
    }
    ...
    private fun displayMovieInfo(movieInfo: com.cactuvi.app.data.models.MovieInfo) {
        ...
        // Set director
        info?.director?.let { directorText.text = it }

        // Set cast
        info?.cast?.let { castText.text = it }
        ...
        movieData?.containerExtension?.let { containerExtension = it }

        movieData?.streamId?.let { streamId = it }
    ...
    private fun showDeleteDownloadDialog() {
        androidx.appcompat.app.AlertDialog.Builder(this)
            ...
            .setPositiveButton("Delete") { _, _ -> deleteDownload() }
