...
import com.iptv.app.data.db.entities.StreamSourceEntity
...
interface StreamSourceDao {
    ...
    suspend fun insert(source: StreamSourceEntity)
    
    @Update
    suspend fun update(source: StreamSourceEntity)
    
    @Delete
    suspend fun delete(source: StreamSourceEntity)
    
    @Query("UPDATE stream_sources SET isActive = 0")
