 ...
 * 
 * SQLite is single-writer, so concurrent writes from Movies/Series/Live cause lock contention.
 * This class uses a shared Mutex to serialize all writes, ensuring:
...
class DbWriter(private val db: AppDatabase) {
    ...
    /**
     * Write movies in large transaction chunks with mutex serialization.
     * Accumulates batches and writes 5000 items per transaction.
     */
    suspend fun writeMovies(entities: List<MovieEntity>) = writeMutex.withLock {
        PerformanceLogger.log("DbWriter: Acquired mutex for movies write (${entities.size} items)")
        ...
        try {
            ...
            PerformanceLogger.log("DbWriter: Movies complete - ${entities.size} items in ${duration}ms ($itemsPerSec/sec)")
        } catch (e: Exception) {
     ...
     */
    suspend fun writeSeries(entities: List<SeriesEntity>) = writeMutex.withLock {
        PerformanceLogger.log("DbWriter: Acquired mutex for series write (${entities.size} items)")
        val start = System.currentTimeMillis()
        ...
        try {
            ...
            val duration = System.currentTimeMillis() - start
            val itemsPerSec = if (duration > 0) (entities.size * 1000L) / duration else 0
            PerformanceLogger.log("DbWriter: Series complete - ${entities.size} items in ${duration}ms ($itemsPerSec/sec)")
        } catch (e: Exception) {
     ...
     */
    suspend fun writeLiveChannels(entities: List<LiveChannelEntity>) = writeMutex.withLock {
        PerformanceLogger.log("DbWriter: Acquired mutex for live channels write (${entities.size} items)")
        val start = System.currentTimeMillis()
        ...
        try {
            ...
            val duration = System.currentTimeMillis() - start
            val itemsPerSec = if (duration > 0) (entities.size * 1000L) / duration else 0
            PerformanceLogger.log("DbWriter: Live channels complete - ${entities.size} items in ${duration}ms ($itemsPerSec/sec)")
        } catch (e: Exception) {
