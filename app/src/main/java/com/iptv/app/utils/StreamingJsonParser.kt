...
/**
 * Utility for streaming large JSON arrays without loading entire response into memory.
 * Parses items one-by-one and processes them in batches to minimize memory pressure.
 */
object StreamingJsonParser {
    ...
    /**
     * Stream parse a JSON array and collect all items (for smaller responses).
     * Use this for categories and other small datasets.
     */
    fun <T> parseArrayFull(
        responseBody: ResponseBody,
        itemClass: Class<T>
    ): List<T> {
        val gson = Gson()
    ...
    /**
     * Check if response is a valid JSON array without fully parsing.
