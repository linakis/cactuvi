...
import com.iptv.app.data.db.entities.toEntity
...
class SourceManager private constructor(context: Context) {
    ...
    suspend fun getAllSources(): List<StreamSource> = withContext(Dispatchers.IO) {
        database.streamSourceDao().getAll().map { it.toModel() }
    }
    
    suspend fun getActiveSource(): StreamSource? = withContext(Dispatchers.IO) {
        database.streamSourceDao().getActive()?.toModel()
    }
    ...
    suspend fun setActiveSource(id: String) = withContext(Dispatchers.IO) {
        database.streamSourceDao().setActive(id)
        prefs.edit().putString(KEY_ACTIVE_SOURCE_ID, id).apply()
    }
    
    suspend fun addSource(source: StreamSource) = withContext(Dispatchers.IO) {
        database.streamSourceDao().insert(source.toEntity())
    }
    
    suspend fun updateSource(source: StreamSource) = withContext(Dispatchers.IO) {
    ...
    suspend fun migrateCurrentCredentialsToSource() = withContext(Dispatchers.IO) {
        ...
        if (server.isNotEmpty() && username.isNotEmpty()) {
            val defaultSource = StreamSource(
                id = "default",
